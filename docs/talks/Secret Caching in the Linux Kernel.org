...
:RESULTS:
Session Keyring
 219659823 --alswrv   1000 65534  keyring: _uid_ses.1000
 905093594 --alswrv   1000 65534   \_ keyring: _uid.1000
 467201697 --alswrv   1000  1000   \_ user: stored:demo
:END:
...
#+RESULTS: print_key
|             |                  |                  |               |         |              |         |     |       |          |         |
| >           | >                | >                | [hex@wpad     | Secret  | Caching      | in      | the | Linux | Kernel]$ | Format: |
| keyctl      | --version        |                  |               |         |              |         |     |       |          |         |
| <desc>      | <data>           | <keyring>        |               |         |              |         |     |       |          |         |
| <gid>       |                  |                  |               |         |              |         |     |       |          |         |
| <uid>       |                  |                  |               |         |              |         |     |       |          |         |
...
| [sep]       |                  |                  |               |         |              |         |     |       |          |         |
| keyctl      | reap             | [-v]             |               |         |              |         |     |       |          |         |
| <timeout>   | <error>          | <keyring>        |               |         |              |         |     |       |          |         |
| <desc>      | [<dest_keyring>] |                  |               |         |              |         |     |       |          |         |
| <desc>      | <info>           | [<dest_keyring>] |               |         |              |         |     |       |          |         |
|             |                  |                  |               |         |              |         |     |       |          |         |
|             |                  |                  |               |         |              |         |     |       |          |         |
| <type>      | <desc>           | [<dest_keyring>] |               |         |              |         |     |       |          |         |
| keyctl      | session          |                  |               |         |              |         |     |       |          |         |
| <arg1>      | <arg2>           | ...]             |               |         |              |         |     |       |          |         |
| [<prog>     | <arg1>           | <arg2>           | ...]          |         |              |         |     |       |          |         |
| <mask>      |                  |                  |               |         |              |         |     |       |          |         |
...
| can         | be               | user             | for           | a       | user-defined | keyring |     |       |          |         |
| :"          |                  |                  |               |         |              |         |     |       |          |         |
...
*** resolve a key
#+name: resolve_key
#+headers: :var DESCRIP="demo"
#+headers: :var DATA="this is some other data"
#+begin_src sh
keyctl request2 user "debug:$DESCRIP" "$DATA"
#+end_src

#+RESULTS: resolve_key
|                                                                                                                  |
| > > [hex@wpad Secret Caching in the Linux Kernel]$ > > > [hex@wpad Secret Caching in the Linux Kernel]$ 70584182 |
...
#+call: del_key(KEYNUM=hello_world)

#+RESULTS:
|                                                                    |
| > > [hex@wpad Secret Caching in the Linux Kernel]$ 1 links removed |
...
Session Keyring
 219659823 --alswrv   1000 65534  keyring: _uid_ses.1000
 905093594 --alswrv   1000 65534   \_ keyring: _uid.1000
 310441321 --alswrv   1000  1000   |   \_ user: test key2
 846204826 --alswrv   1000  1000   \_ user: stored:demo
  70584182 --alswrv   1000  1000   \_ user: debug:demo
1 links removed
Session Keyring
 219659823 --alswrv   1000 65534  keyring: _uid_ses.1000
 905093594 --alswrv   1000 65534   \_ keyring: _uid.1000
...
#+begin_example
  create  user    stored:debug:*  *        /usr/local/sbin/debug_key_request.sh   /usr/local/sbin/storedresolver %k %u %g %S %c
#+end_example

#+call: create_stored_demo(ID="debug:demo")

#+RESULTS:
: nil

#+call: del_key(KEYNUM=create_stored_demo(ID="debug:demo"))
